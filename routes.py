...
from loginHelper import StringChecker, UsedEmails, UsedUsernames

from datetime import datetime

import os;

def Init(app, staticPath):
    @app.route("/")
    def Home():
        return render_template("index.html", user = current_user, homeFoods = Search(None, "top", "")[:4])
    ...
    def FoodPage(foodID):
        ...
        return render_template("post.html", food = food, user = current_user, cookTok = False)

    @app.route("/CookTok/<int:foodID>")
    def CookTok(foodID):
        foodID = foodID + 1

        highestID = 0
        for food in Food.query.all():
            if food.id > highestID: highestID = food.id
        print(highestID)
        ...
        return render_template("post.html", food = Food.query.get(foodID), user = current_user, cookTok = True)
